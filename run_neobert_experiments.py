"""
NeoBERT as Third Backbone

1. Fine-tune NeoBERT on PhraseBank (standalone accuracy as upper bound)
2. Experiment B: KD routing with NeoBERT as teacher
3. Experiment D: Multi-exit with NeoBERT backbone (28 layers, exits at 7/14/21)

Usage:
    python run_neobert_experiments.py --phase finetune   # Fine-tune NeoBERT
    python run_neobert_experiments.py --phase kd          # KD routing (Exp B)
    python run_neobert_experiments.py --phase multi-exit  # Multi-exit (Exp D)
    python run_neobert_experiments.py --phase all         # Run all phases
"""

# ============================================================
# Mock xformers.ops.SwiGLU before any NeoBERT import
# NeoBERT's custom code does `from xformers.ops import SwiGLU`
# but xformers is not compatible with our torch+CUDA version.
# SwiGLU is just: silu(W1·x) * W3·x -> W2
# ============================================================
import sys
import types
...
class _SwiGLU(nn.Module):
    """Drop-in replacement for xformers.ops.SwiGLU.

    xformers.ops.SwiGLU fuses w1 and the gate into a single w12 linear,
    and uses w3 as the output projection:
        x12 = w12(x);  x1, x2 = chunk(x12);  out = w3(silu(x1) * x2)
    """
...
# Inject mock xformers modules
_xformers_ops = types.ModuleType("xformers.ops")
_xformers_ops.SwiGLU = _SwiGLU
...
NEOBERT_SAVE_DIR = Path("neobert_finetuned")
...
# ============================================================
# Phase 1: Fine-tune NeoBERT
# ============================================================

def load_neobert_for_classification(model_name: str, num_labels: int = 3):
    """Load NeoBERT with classification head via trust_remote_code."""
    config = AutoConfig.from_pretrained(
        model_name, trust_remote_code=True,
        num_labels=num_labels,
    )
    # The model repo provides NeoBERTForSequenceClassification
    # but AutoModelForSequenceClassification may not find it.
    # Import the class directly from the dynamic module.
    ...
    # Load pretrained encoder weights (ignore classification head)
    base_model = AutoModel.from_pretrained(model_name, trust_remote_code=True)
    model.model.load_state_dict(base_model.state_dict(), strict=True)
    del base_model
    torch.cuda.empty_cache()
    return model


def finetune_neobert(
    train_texts, train_labels, val_texts, val_labels,
    cfg: FinetuneConfig, save_dir: Path,
) -> Dict:
    """Fine-tune NeoBERT on PhraseBank."""
    print(f"\n=== Fine-tuning {NEOBERT_MODEL} ===")
    tokenizer = AutoTokenizer.from_pretrained(NEOBERT_MODEL, trust_remote_code=True)
    model = load_neobert_for_classification(NEOBERT_MODEL, num_labels=len(LABEL_LIST))
    model.to(DEVICE)
...
# ============================================================
# Phase 2: KD Routing (Experiment B) with NeoBERT teacher
# ============================================================

def predict_with_neobert(
    model_path: str, texts: List[str], batch_size: int = 32,
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Predict with a saved NeoBERT model. Returns (labels, confidences, logits)."""
    ...
    try:
        ...
        model = cls.from_pretrained(model_path, config=config)
    except Exception:
        # Fallback: load from original repo class with local weights
        cls = get_class_from_dynamic_module(
            "model.NeoBERTForSequenceClassification",
            NEOBERT_MODEL,
            trust_remote_code=True,
    ...
    # NeoBERT classification head outputs logits in order [0,1,2] = our label order
    # since we trained it with LABEL2ID mapping
    all_logits = []
...
def run_kd_experiment(
    ...
    batch_size: int = 16,
    results_dir: Path = Path("results_neobert_kd"),
) -> Dict:
    """Experiment B: DistilBERT student with NeoBERT teacher."""
    print("\n=== Experiment B: KD with NeoBERT Teacher ===")
    results_dir.mkdir(exist_ok=True)

    # Get teacher predictions and logits
    print("  Getting NeoBERT teacher predictions...")
    teacher_preds, teacher_confs, teacher_logits_raw = predict_with_neobert(
    ...
    print(f"  NeoBERT teacher: acc={teacher_acc:.4f}, f1={teacher_f1:.4f}")

    # Train DistilBERT student with KD from NeoBERT
    from hybrid_sentiment_v2 import DistilStudent, TrainConfig
    ...
    # Phase 2: KD with NeoBERT logits
    print("\n  Phase 2: KD with NeoBERT teacher logits...")
    train_teacher_preds, _, train_teacher_logits = predict_with_neobert(
        teacher_path, train_texts, batch_size=batch_size
    )
    val_teacher_preds, _, val_teacher_logits = predict_with_neobert(
    ...
    out_json = results_dir / "neobert_kd_results.json"
...
# ============================================================
# Phase 3: Multi-Exit with NeoBERT backbone (Experiment D)
# ============================================================
...
def run_multi_exit_experiment(
    ...
    batch_size=32,
    head_epochs=30,
    results_dir=Path("results_neobert_multi_exit"),
) -> Dict:
    """Experiment D: Multi-exit with NeoBERT backbone (28 layers)."""
    print("\n=== Experiment D: Multi-Exit NeoBERT ===")
    results_dir.mkdir(exist_ok=True)

    tokenizer = AutoTokenizer.from_pretrained(model_path, trust_remote_code=True)

    # Load the classification model to get the base model
    config = AutoConfig.from_pretrained(model_path, trust_remote_code=True)
    from transformers.dynamic_module_utils import get_class_from_dynamic_module
    try:
        cls = get_class_from_dynamic_module(
    ...
    # NOTE: NeoBERT's hidden_states[i] = output of transformer layer i (0-indexed)
    # No embedding layer in hidden_states, so hidden_states[6] = layer 7 output
    def extract_features(texts, bs=batch_size):
        ...
        for i in range(0, len(texts), bs):
            ...
            # out.hidden_states is list of 28 tensors (one per layer)
            for l in exit_layers:
                # hidden_states[l-1] = output of layer l (1-indexed)
                layer_idx = l - 1
                layer_cls[l].append(out.hidden_states[layer_idx][:, 0, :].cpu().numpy())
            final_logits_all.append(out.logits.cpu().numpy())
    ...
    print(f"\n  Full NeoBERT:  acc={full_acc:.4f}, F1={full_f1:.4f}")
    ...
    out_json = results_dir / "neobert_multi_exit_results.json"
    with open(out_json, "w") as f:
        json.dump(make_json_safe(results), f, indent=2)
    print(f"  Saved to {out_json}")
...
def main():
    parser = argparse.ArgumentParser(description="NeoBERT backbone experiments")
    parser.add_argument("--phase", type=str, default="all",
                       choices=["finetune", "kd", "multi-exit", "all"])
    parser.add_argument("--data-dir", type=str, default="FinancialPhraseBank-v1.0")
    parser.add_argument("--results-dir", type=str, default="results_neobert")
    ...
    if args.phase in ("finetune", "all"):
        ...
        print(f"\n  NeoBERT standalone: acc={acc:.4f}, F1={f1:.4f}, ECE={ece:.4f}")

    # Phase 2: KD experiment
    if args.phase in ("kd", "all"):
        if not model_save_dir.exists():
            print("ERROR: Fine-tuned NeoBERT not found. Run --phase finetune first.")
            return
    ...
    out_json = out_dir / "all_neobert_results.json"
    with open(out_json, "w") as f:
