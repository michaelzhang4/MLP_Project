...
DEBERTA_SAVE_DIR = Path("deberta_finetuned")
...
def predict_with_model(
    ...
    """Predict with a saved model. Returns (labels, confidences, logits)."""
...
def run_kd_experiment(
    ...
    teacher_preds, teacher_confs, teacher_logits_raw = predict_with_model(
    ...
    train_teacher_preds, _, train_teacher_logits = predict_with_model(
        teacher_path, train_texts, batch_size=batch_size
    )
    val_teacher_preds, _, val_teacher_logits = predict_with_model(
...
def run_multi_exit_experiment(
    ...
    # Simple entropy-based routing (no XGBoost dependency for cleaner DeBERTa comparison)
