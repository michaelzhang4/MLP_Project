...
def plot_pareto_frontier(results: dict, out_dir: Path):
    ...
    ax.plot(lf, accs, "o-", color="steelblue", markersize=2, label="Entropy Router", alpha=0.8)
    ...
    ax.set_title("Pareto Frontier: Entropy Router")
...
def main():
    json_path = sys.argv[1] if len(sys.argv) > 1 else str(Path(__file__).resolve().parent / "results_full" / "all_results_v5_multi_exit.json")
    ...
    print(f"Generated 7 plots in {out_dir}")
