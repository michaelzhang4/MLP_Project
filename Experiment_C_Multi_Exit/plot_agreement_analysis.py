...
def main():
    ...
    _root = _here.parent
    out_dir = _here / "agreement_analysis"
    ...
    # V1: VADER Cascade
    v1_path = _root / "all_results" / "results" / "all_results.json"
    if v1_path.exists():
    ...
    # V2: KD Student
    v2_path = _root / "all_results" / "results_v2_full_kd" / "all_results_v2.json"
    if v2_path.exists():
    ...
    # V5: Multi-Exit PABEE
    v5_path = _root / "all_results" / "results_v5_full" / "all_results_v5_multi_exit.json"
    if v5_path.exists():
    ...
    deb_kd = _root / "results_deberta" / "kd" / "deberta_kd_results.json"
    if deb_kd.exists():
        dk = load_json(deb_kd)
    ...
    deb_me = _root / "results_deberta" / "multi_exit" / "deberta_multi_exit_results.json"
    if deb_me.exists():
        dm = load_json(deb_me)
    ...
    # NeoBERT results
    neo_path = _root / "results_neobert" / "all_neobert_results.json"
    if neo_path.exists():
