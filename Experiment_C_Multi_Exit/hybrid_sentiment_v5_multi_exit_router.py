"""
Variant 5: Multi-Exit FinBERT with Patience-Based Entropy Routing.

Architecture:
1. GPU inference with proper benchmarking
...
4. Entropy-based routing: exit early when softmax entropy is below threshold
...
class MultiExitFinBERT:
    ...
    def benchmark_multi_exit(
        ...
        all agree on the same prediction AND entropy is below threshold tau.
        """
        all_preds = []
        all_exit_layers = []
        ...
        for i in range(0, len(texts), batch_size):
            ...
            for li, layer in enumerate(self.exit_layers):
                ...
                # Entropy-based exit decision for undecided samples
                ...
                probs = _softmax_np(undecided_logits)
                entropy = _compute_entropy(probs)

                # Decision: exit if entropy low AND patience met
                can_exit = (entropy <= tau) & (undecided_agreement >= patience)
...
def _select_tau(
    ...
    """Sweep entropy threshold tau and find optimal operating point for multi-exit."""
    full_acc = accuracy_score(labels, final_preds)

    # Pre-compute entropy and predictions for each exit layer
    exit_data = {}
    for layer in exit_layers:
        ...
        agreement = np.ones(len(labels), dtype=float)
    ...
    for tau in np.linspace(tau_min, tau_max, tau_steps):
        ...
        for layer in exit_layers:
            ed = exit_data[layer]
            can_exit = (~decided) & (ed["entropy"] <= tau) & (ed["agreement"] >= patience)
            hybrid_preds[can_exit] = ed["preds"][can_exit]
...
def main():
    ...
    # ================================================================
    # Validation threshold search (entropy-based)
    # ================================================================
    print("\n=== Validation entropy threshold search ===")
    ...
    # Hybrid multi-exit predictions (entropy-based routing with patience)
    ...
    for layer in exit_layers:
        ...
        can_exit = (~decided) & (entropy <= tau) & (agreement >= args.patience)
    ...
    # ================================================================
    results = {
        "version": "v5_multi_exit_pabee_entropy",
        "config": vars(args),
        "model": {
            ...
            "routing": "entropy_threshold_with_patience",
        },
        "training": {
    ...
    print(f"Entropy threshold:  tau={tau:.3f}")
