"""
Variant 4: Quantized early-exit FinBERT with learned routing.

Architecture:
- Single FinBERT backbone (INT8 dynamic quantization on CPU)
- Stage 1 runs layers 1-4 and produces early logits
- XGBoost router predicts whether early output is reliable from:
  [layer4 logits + hedge-word count]
- Reliable samples exit at layer 4, otherwise continue through layers 5-12
"""
...
class ConstantReliabilityRouter:
    """Fallback router when training labels contain one class only."""
...
def _hedge_count_and_hits(text: str) -> Tuple[float, List[str]]:
    tokens = re.findall(r"[a-zA-Z']+", text.lower())
    hits = [tok for tok in tokens if tok in HEDGE_WORDS]
    return float(len(hits)), hits


def _build_router_features(layer_logits: np.ndarray, texts: List[str]) -> Tuple[np.ndarray, np.ndarray]:
    hedge_counts = np.array([_hedge_count_and_hits(t)[0] for t in texts], dtype=float)
...
def _router_prob_correct(router, router_features: np.ndarray) -> np.ndarray:
    probs = router.predict_proba(router_features)
...
def _select_tau(
    ...
    for tau in np.linspace(tau_min, tau_max, tau_steps):
        use_early = router_scores >= float(tau)
    ...
    def benchmark_hybrid_runtime(
        ...
        with torch.no_grad():
            for i in range(0, len(texts), batch_size):
                ...
                t0 = time.time()
                router_features, _ = _build_router_features(early_logits, batch)
                prob_correct = _router_prob_correct(router, router_features)
                use_early = prob_correct >= tau
...
def _run_demo_case(
    ...
    with torch.no_grad():
        ...
        router_features, hedge_counts = _build_router_features(early_logits, [text])
        prob_correct = float(_router_prob_correct(router, router_features)[0])
        use_early = prob_correct >= tau
    ...
    _, hedge_hits = _hedge_count_and_hits(text)
    return {
        "text": text,
        ...
        "hedge_count": float(hedge_counts[0]),
        "hedge_hits": hedge_hits,
        "router_prob_reliable": prob_correct,
        "tau": float(tau),
...
def main():
    parser = argparse.ArgumentParser(
        description="Hybrid v4: Quantized Early-Exit FinBERT with XGBoost routing"
    )
    ...
    parser.add_argument("--router-min-child-weight", type=float, default=1.0)
    parser.add_argument("--router-reg-lambda", type=float, default=1.0)
    ...
    print("=== Train reliability router (XGBoost) ===")
    train_router_features, _ = _build_router_features(train_early_logits, train_texts)
    train_reliable = (train_early_preds == train_labels).astype(int)
    reliable_rate = float(train_reliable.mean())

    feature_names = [
        "layer4_logit_negative",
        "layer4_logit_neutral",
        "layer4_logit_positive",
        "hedge_count",
    ]

    unique_train_targets = np.unique(train_reliable)
    if len(unique_train_targets) < 2:
        router = ConstantReliabilityRouter(
            prob_correct=float(unique_train_targets[0]),
            n_features=train_router_features.shape[1],
        )
        router_type = "constant"
        print("Router target collapsed to one class; using constant router fallback.")
    else:
        router = XGBClassifier(
            ...
            min_child_weight=args.router_min_child_weight,
            reg_lambda=args.router_reg_lambda,
            random_state=args.seed,
            n_jobs=-1,
        )
        router.fit(train_router_features, train_reliable)
        router_type = "xgboost"
    ...
    val_router_features, _ = _build_router_features(val_early_logits, val_texts)
    val_router_scores = _router_prob_correct(router, val_router_features)
    val_full_preds, _, _ = model.predict_full(val_texts, batch_size=args.batch_size)

    tau, best_point, sweep, val_full_acc = _select_tau(
        router_scores=val_router_scores,
    ...
    test_router_features, _ = _build_router_features(test_early_logits, test_texts)
    test_router_scores = _router_prob_correct(router, test_router_features)
    full_preds, full_logits, full_time = model.predict_full(
        test_texts, batch_size=args.batch_size
    )

    use_early = test_router_scores >= tau
    ...
    n = len(test_texts)
    results = {
        "version": "v4_quantized_early_exit_xgboost_router",
        "config": vars(args),
        "model": {
